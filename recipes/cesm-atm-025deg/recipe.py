...
variables = ['LHFLX', 'SHFLX', 'FLDS', 'FSNS', 'PSL', 'TAUX', 'TAUY', 'TS', 'U10']


def make_filename(variable, time):
    return (
        'https://tds.ucar.edu/thredds/fileServer/datazone/campaign/cesm/collections/ASD/'
...
target_chunks = {
    'time': 73
}  # Full spatial domain: "lat": 768, "lon": 1152, --> if not specified, will include full extent

recipe = XarrayZarrRecipe(
    file_pattern=pattern, target_chunks=target_chunks, subset_inputs={'time': 5}
)  # set 5 chunks per year, each with time of length 73 (to total 365)
