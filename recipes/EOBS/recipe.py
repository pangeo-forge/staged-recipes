...
tg_tn_tx_rr_hu_pp_pattern = FilePattern(
    make_filename, ConcatDim("time", keys=[""]), MergeDim(name="variable", keys=["tg", "tn", "tx", "rr", "pp", "hu"])
)
...
tg_tn_tx_rr_hu_pp_recipe = XarrayZarrRecipe(
    file_pattern=tg_tn_tx_rr_hu_pp_pattern, target_chunks=target_chunks,
    subset_inputs=subset_inputs
)

qq_pattern_recipe = XarrayZarrRecipe(
    file_pattern=qq_pattern, target_chunks=target_chunks,
    subset_inputs=subset_inputs
)

fg_pattern_recipe = XarrayZarrRecipe(
    file_pattern=fg_pattern, target_chunks=target_chunks,
    subset_inputs=subset_inputs
)
