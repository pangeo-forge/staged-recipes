...
def get_surf_url(region, dim='surf', dates=surf_dates,):
    ...
    input_urls = [
        pattern.format(dim=dim, reg=region, yymmdd=day.strftime("%Y-%m-%d"))
        for day in dates
    ]
    return input_urls
...
int_dates = pd.concat(
    [int_dates, pd.date_range("2009-02-01", "2009-05-01", freq="1D").to_series()],
    ignore_index=True
)
...
def get_int_url(region, dates=int_dates,):
    ...
    input_urls = [
        pattern.format(reg=region, yymmdd=day.strftime("%Y-%m-%d"))
        for day in dates
    ]
    return input_urls
